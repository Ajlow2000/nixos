...
fn main() {
    ...
    for path in &paths {
        ...
        if let Some(&created) = active.get(&name) {
            ...
            lines.push(format!("[{}]\t{}", uptime, path.display()));
        } else {
    ...
    for (session, &created) in &active {
        if !manifest_sessions.contains_key(session) {
            let uptime = format_uptime(now.saturating_sub(created));
            lines.push(format!("[{}]\t@ {}", uptime, session));
        }
    }
    ...
    // Uptime is the first column; path/session name is after the tab
    let data = selected.split('\t').nth(1).unwrap_or(&selected);
